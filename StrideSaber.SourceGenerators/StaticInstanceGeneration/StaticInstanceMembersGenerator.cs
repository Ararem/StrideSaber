	...
	{{");
		...
		public static {field.Type} {field.Name} => {instanceName}.{field.Name};");
					...
					case IPropertySymbol {IsIndexer: false} prop:
						...
						{
							Log($"\t\tSkipping init-only property  {prop} ");
							break;
						}
		...
		{{");
			...
			set => {instanceName}.{prop.Name} = value;");
					...
					case IPropertySymbol {IsIndexer: true} prop:
						Log($"\t\tSkipping unsupported indexer {prop}");
						break;
					...
					case IMethodSymbol {MethodKind: MethodKind.Ordinary} method:
						...
						string returnType = "";
						if (method.IsAsync) returnType += "async "; //We don't add `async` to async void's because they're special (more like special needs)
			...
			=> {(method.IsAsync && !method.ReturnsVoid ? "await " : "")}{instanceName}.{method.Name}{genericArgs}({methodCallArgs});");
					...
					default:
						...
						Log(member.Dump(new DumpOptions
						{
								...
								TrimTrailingColonName = false,
								UseTypeFullName = true,
								MaxLevel = 5
						}));
						Log(JsonConvert.SerializeObject(member, new JsonSerializerSettings()
						{
